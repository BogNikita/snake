...
var delete_bomb_timer; // Таймер для удаления бомб
var score = 0; // Результат
var countBomb=0; // Количество бомб
function init() {
    ...
    */
    ...
    score.id = "scoreOnline"
    wrap.appendChild(score);
    
// Отслеживание клавиш клавиатуры
    addEventListener('keydown', changeDirection);
    score.innerText = "Ваш текущий счет: 0";
}
...
function startGame() {
    ...
    delete_bomb_timer = setInterval(deleteBomb, 7500)
    setInterval(console.log(SNAKE_SPEED), 2000)
}


function deleteBomb(){
    var bomb = document.getElementsByClassName("bomb-unit");
    countBomb--;
    bomb[Math.round(Math.random()*countBomb)].classList.remove("bomb-unit")
}
/**
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        createFood();
        score++;
        document.getElementById('scoreOnline').innerText="Ваш текущий счет: " + score;
    }
    return check;
}
...
function finishTheGame() {
    ...
    clearInterval(delete_bomb_timer);
